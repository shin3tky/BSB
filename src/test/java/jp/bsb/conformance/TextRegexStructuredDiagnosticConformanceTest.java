...
/** 表示文字列の手前にある、機械可読な文字列・正規表現診断の全フィールドを検証します。 */
class TextRegexStructuredDiagnosticConformanceTest {
  @TestFactory
  List<DynamicTest> everyFailureAndWarningHasTheSpecifiedStructuredDiagnostic() throws IOException {
    ...
    Map<String, CaseSpec> cases =
        TextRegexConformanceData.loadCases().cases().stream()
  ...
  private static void compare(String label, CaseSpec caseSpec, DiagnosticSpec spec)
      throws IOException {
    byte[] source = TextRegexConformanceData.generateCaseSource(caseSpec).bytes();
    ...
    TextRegexDiagnosticSupport.assertStructuredDiagnostic(
